...
int main(void){
	...
	int i = 1;
	float p = 0, q = 0, r = 0, s = 0; 	//p, q, r e s são os coeficientes da equação
	float E = 0, x = 0, a = 0, b = 0, y = 0, Ea = 999; 
	float f_x = 0, f_a = 0, f_b = 0; 	//Funções a serem calculadas
	
	printf("\t\t-----------------------------------\n");
	printf("\t\tMétodo da Bissecção - SMS Produções\n");
	printf("\t\t-----------------------------------\n"); 
	...
	printf("b: ");
	scanf("%f", &b);
	f_a =  p*pow(a, 3) + q*pow(a, 2) + r*pow(a, 1) + s; 	//Cálculo para saber se o intervalo contém a raiz
	f_b =  p*pow(b, 3) + q*pow(b, 2) + r*pow(b, 1) + s;
	
	if(f_a*f_b < 0){ 	//Se f(a)*f(b) < 0, então esse intervalo [a, b] contém a raiz
		printf("\nInforme a precisão: ");
		scanf("%f", &E);	
		while(Ea > E){ 	//Enquanto o erro relativo aproximado for maior que o erro informado, continue a calcular
			...
			f_a = p*pow(a, 3) + q*pow(a, 2) + r*pow(a, 1) + s; 	//Calcula a função f(a)
			printf("f(a) = %.10f \n", f_a);
			
			x = (a + b)/2; 	//A raiz é a média do intervalo que contém a raiz
			printf("x = %.10f \n", x);
			
			f_x =  p*pow(x, 3) + q*pow(x, 2) + r*pow(x, 1) + s; 	//Calcula a função f(x)
			printf("f(x) = %.10f \n", f_x);
			...
			Ea = fabs(100*(x - y)/x); 	//Cálculo do Erro Ea = | (x_n - x_v)/x_n |
